...
def convert_json_to_csv(json_file, csv_file, columns_to_keep=None):
    ...
    if isinstance(first_val, dict) and 'data' in first_val and isinstance(first_val['data'], list):
    ...
    if columns_to_keep:
        ...
        if missing:
            print(f"Warning: The following requested columns were not found in the data: {', '.join(missing)}")
    else:
    ...
    print(f"Writing {len(rows_to_write)} rows to {csv_file} (Columns: {len(headers)})...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert GitHub analytics JSON caches to CSV.")
    parser.add_argument("input", nargs="?", help="The JSON file to convert (e.g., followers_cache.json)")
    parser.add_argument("--columns", "-c", help="Comma-separated list of columns to keep (e.g., 'login,name,location')")
